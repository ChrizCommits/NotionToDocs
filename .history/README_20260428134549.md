...
| Notion (Source) | Google Document Document | Gemini Recipe Suggestion |
|-----------------|--------------------------|--------------------------|
...
2. Prepare Notion:
   ...
   * Create an additional database called `NotionToDocs Sync`  
...
Make sure Gemini has access to your document (e.g. via Google Drive permissions or by opening the document in Gemini).

Then simply reference your recipes in a prompt, for example:
- "I need a quick recipe for lunch"
- "Suggest a breakfast recipe based on these ingredients: ..."
...
*This step is optional. The sync to the Google Doc works independently without using Gemini.*
